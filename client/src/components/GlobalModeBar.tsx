...
import { FlaskConical, Radio, X, ArrowLeftRight } from 'lucide-react';
...
export function GlobalModeBar({ className }: GlobalModeBarProps) {
  const { isSandbox, isLive, exitSandbox, enterSandbox } = useMode();

  if (isSandbox) {
    return (
      <div 
        className={cn(
          "w-full bg-gradient-to-r from-cyan-600 via-teal-500 to-cyan-600 text-white px-4 py-2.5 shadow-lg shadow-cyan-500/30",
          className
        )} 
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 rounded bg-white/20 text-[10px] font-bold uppercase">Training</span>
              </div>
